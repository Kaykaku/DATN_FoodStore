app.controller("dashboard-ctrl", function($scope, $http, $location, $filter) {
    $scope.firstRow = [];
    ...
    $scope.initialize = function() {
        //getFirstRow Content
        $http.get(url + "/rest/summary/firstRow").then(resp => {
                $scope.firstRow = resp.data;
            })
            //getSecondRow Content
        $http.get(url + "/rest/summary/secondRow").then(resp => {
                //revenue last 7days
            ...
            //getThirdRow Content
        $http.get(url + "/rest/summary/thirdRow").then(resp => {
            //product sold by categories
        ...
        $http.get(url + "/rest/summary/fourthRow").then(resp => {
